...
public class ClassifierCharacterizations 
{
	/*
			...
			) throws Exception
	{
		...
		ArrayList<EvaluateAttributeCombinaison> evalCombinations = new ArrayList<EvaluateAttributeCombinaison>(); 
		for( int i = 0; i < attributeIndex.length; i++ )
		{			
			if( verbose )
		...
		final EvaluateAttributeCombinaison bestCombination = pickBestCombination( evalCombinations );
	 ...
	 */
	public static ArrayList<EvaluateAttributeCombinaison> combineAttributes(
			final AbstractClassifier classifier,
			final Instances data, 
			...
			final int chooseNr ) throws Exception
	{
		...
		Instances subset;
		Evaluation eval; 
		ArrayList<EvaluateAttributeCombinaison> evalList = new ArrayList<EvaluateAttributeCombinaison>();  
		for (ICombinatoricsVector<Integer> combination : gen) 
		{
			...
			evalList.add( new EvaluateAttributeCombinaison( combination, eval) ); 
		}
	 ...
	 */
	private static EvaluateAttributeCombinaison pickBestCombination( final ArrayList<EvaluateAttributeCombinaison> evalCombinations )
	{
		double minError = Double.MAX_VALUE; 
		EvaluateAttributeCombinaison bestCombination = null; 
		
		int i = 0; 
		double currentError; 
		for( EvaluateAttributeCombinaison combination : evalCombinations ) 
		{
